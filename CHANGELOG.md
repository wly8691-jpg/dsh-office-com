...
公开发布的 README 里出现了一个内部项目名（「不依赖 KnowLP 或任何记忆系统」），
npm 包页上就那么挂着。改为「不依赖任何记忆系统」——语义完全一样，只是不点名。

- README 的能力边界表：去掉内部项目名
- `lib/index.mjs` 的 `cleanEnv()` 注释：「Hermes/坏 venv」→「外部进程透传坏 venv」
  （这条是历史遗留，从 0.1.0 就在）
- **git 历史不动**——按纪律历史遗留不改写；已发布版本的 npm 包也改不了，
  只能靠新版本覆盖页面

顺带做了一次全仓卫生自查（KnowLP / Hermes / 否极泰来 / 产研汇 / 词元 / 奇门 / 海家 /
私人路径等），工作树已干净。`dev.patch.yml` 含本机路径但它在 `.gitignore` 里，
不进仓库也不进 npm 包，无需处理。
