...
const PREAMBLE = `import json, pythoncom
pythoncom.CoInitialize()
...
def _finish(ok):
    ...
    if ok and c.get('want_save') and may_save:
        try:
            _ctx['wb'].Save()
            _ctx['saved'] = True
        except Exception as e:
...
const MODE_PARAMS = {
  mode: {
    type: 'string', enum: MODES, required: false,
    description: '执行模式：preview 只预演不落笔 / managed 打开-改-存-关 / attached 改当前运行实例且不关不存。缺省按有无 path 推断（有=managed，无=attached）',
  },
  save: { type: 'boolean', required: false, description: '是否落盘。managed 缺省 true、attached 缺省 false。高风险参数，显式声明才生效' },
  close: { type: 'boolean', required: false, description: '是否关闭。managed 缺省 true（自己开的那份）；attached 下无效——用户的工作簿永不代关' },
  overwrite: { type: 'boolean', required: false, description: '是否允许覆盖既有内容（如已存在的总账工作表）。涉及覆盖时必填 true，否则返回 OVERWRITE_NOT_CONFIRMED' },
  confirm: { type: 'boolean', required: false, description: '高风险动作确认位（如运行 VBA 宏）。缺省 false = 拒绝执行' },
}
