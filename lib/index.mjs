...
const CODE_VBA_RUN = comCode([
  ...
  '    _mods = None',
  "_emit_preview(_plan({'macro': args['macro'], 'module_candidates': _mods, 'note': '宏的具体效果无法预演，preview 只说明将要运行哪个宏以及确认位是否就位'}))",
  '_mark_changed()',
...
const CODE_PIVOT_REFRESH = comCode([
  ...
  '    _p.RefreshTable()',
  '    count += 1',
  "output = json.dumps({'refreshed': count, 'external_sources': _ext}, ensure_ascii=False)",
  '_finish(True)',
])
